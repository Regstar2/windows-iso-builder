...
Для release-bound изменений текущий head должен пройти:
...
- root `VERSION` и GUI `<Version>` равны `1.0.0`;
- GUI FileVersion/AssemblyVersion используют numeric `1.0.0.0`;
...
| MAN-UPD-001 | Update check | Проверить Stable/Prerelease update check через выбранную policy. | Network failure не ломает приложение; URL остаётся official `https://github.com/Regstar2/windows-iso-builder`. | | NOT RUN | Внешний acceptance требует доступного GitHub release channel. |
...
Built-in safety scan проверяет текущие tracked files и current release package. Это **не Git-history audit**. Перед stable release требуется отдельный full-history secret scan.
