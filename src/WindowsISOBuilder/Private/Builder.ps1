...
function Get-WibResponsePreview {
    ...
    catch { return "<не удалось прочитать ответ: $($_.Exception.Message)>" }
}
...
function Test-WibUupPackageArchive {
    ...
    try {
        ...
        if ($missing.Count -gt 0) { return [pscustomobject]@{ IsValid=$false; Reason="В ZIP отсутствуют: $($missing -join ', ')"; Entries=$entries } }
...
function Download-WibUupPackage {
    ...
    if (Test-Path -LiteralPath $DestinationZip) {
        ...
        if ($existing.IsValid) { Write-WibInfo "Используется кешированный пакет UUP dump: $DestinationZip"; return }
    ...
    for ($attempt=1; $attempt -le $Attempts; $attempt++) {
        ...
        try {
            ...
            if ($attempt -lt $Attempts) { Start-Sleep -Seconds (5 * $attempt); continue }
            throw "Не удалось загрузить пакет UUP dump: $lastReason"
        }
        $validation = Test-WibUupPackageArchive -Path $DestinationZip
        if ($validation.IsValid) { Write-WibInfo "Получен корректный пакет: $((Get-Item -LiteralPath $DestinationZip).Length) байт."; return }
...
function Invoke-WibUupDownloadScript {
    ...
    try {
        & $powerShellExecutable -NoLogo -NoProfile -ExecutionPolicy Bypass -Command $childCommand |
...
function Get-WibIsoMetadata {
    ...
    if (-not (Get-Command Mount-DiskImage -ErrorAction SilentlyContinue)) { $result.Warning='Mount-DiskImage недоступен; внутреннее содержимое ISO не проверено.'; return [pscustomobject]$result }
    $diskImage = $null
    try {
    ...
    catch { $result.Warning = $_.Exception.Message }
    finally { if ($null -ne $diskImage) { Dismount-DiskImage -ImagePath $IsoPath -ErrorAction SilentlyContinue | Out-Null } }
    return [pscustomobject]$result
}

function Save-WibJobState {
    param([Parameter(Mandatory = $true)][string]$Path, [Parameter(Mandatory = $true)][string]$Stage, [Parameter(Mandatory = $true)]$Plan, [string]$Message = '')
    Write-WibJsonFile -Path $Path -Depth 20 -Value ([ordered]@{ stage=$Stage; updatedAt=(Get-Date).ToString('o'); message=$Message; plan=$Plan })
}
...
function Start-WibElevatedPlan {
    ...
    $arguments = @('-NoLogo','-NoProfile','-ExecutionPolicy','Bypass','-File',(Quote-WibCommandArgument -Value $entryScript),'-PlanFile',(Quote-WibCommandArgument -Value $planPath))
    ...
    try {
        ...
        if ($process.ExitCode -ne 0) { throw "Повышенный процесс завершился с кодом $($process.ExitCode). План: $planPath" }
    }
    catch { throw "Не удалось запустить сборку с правами администратора: $($_.Exception.Message)" }
...
function Invoke-WibBuildPlanCore {
    ...
    try {
        Start-Transcript -LiteralPath $logPath -Force | Out-Null; $transcriptStarted = $true
        Write-WibStage 'План сборки'
        Write-Host "Сборка:      $($Plan.Build.Title)"; Write-Host "Номер:       $($Plan.Build.Build)"; Write-Host "Архитектура: $($Plan.Build.Architecture)"; Write-Host "Язык:        $($Plan.Language)"; Write-Host "Редакции:    $(@($Plan.Editions) -join ', ')"; Write-Host "Формат:      install.$(([string]$Plan.ImageFormat).ToLowerInvariant())"; Write-Host "Результат:   $outputDirectory"; Write-Host "Кеш:         $cacheDirectory"; Write-Host "Лог:         $logPath"
        Save-WibJobState -Path $statePath -Stage 'downloading-package' -Plan $Plan
        Write-WibStage 'Получение пакета UUP dump'; Download-WibUupPackage -Plan $Plan -DestinationZip $packageZip
        ...
        if (-not $downloadScript) { Write-WibInfo 'Распаковка пакета конвертации...'; Expand-Archive -LiteralPath $packageZip -DestinationPath $workDirectory -Force; $downloadScript=Get-ChildItem -LiteralPath $workDirectory -Filter 'uup_download_windows.cmd' -File -Recurse | Select-Object -First 1 }
        if (-not $downloadScript) { throw 'В пакете отсутствует uup_download_windows.cmd.' }
        $packageDirectory=$downloadScript.Directory.FullName; $configPath=Join-Path $packageDirectory 'ConvertConfig.ini'
        ...
        Write-WibStage 'Загрузка файлов Microsoft и создание ISO'; Write-WibInfo 'aria2 продолжит неполные загрузки из сохранённого рабочего каталога.'
        ...
        if (-not $iso) { throw "Конвертер завершился, но ISO-файл не найден в рабочем каталоге: $workDirectory. Подробный лог: $converterLogPath" }
        if ($iso.Length -lt 1GB) { throw "Созданный ISO подозрительно мал: $($iso.Length) байт." }
        $editionPart=ConvertTo-WibSafeFilePart -Value (@($Plan.Editions) -join '+'); $versionPart=if ([string]::IsNullOrWhiteSpace([string]$Plan.Build.VersionLabel)) { 'build' } else { [string]$Plan.Build.VersionLabel }
        ...
        $destinationIso=Join-Path $outputDirectory $fileName; Move-Item -LiteralPath $iso.FullName -Destination $destinationIso -Force
        Save-WibJobState -Path $statePath -Stage 'validating' -Plan $Plan; Write-WibStage 'Проверка результата'
        $hash=(Get-FileHash -LiteralPath $destinationIso -Algorithm SHA256).Hash.ToLowerInvariant(); [IO.File]::WriteAllText("$destinationIso.sha256.txt", "$hash  $fileName`r`n", [Text.Encoding]::ASCII)
        $isoMetadata=Get-WibIsoMetadata -IsoPath $destinationIso; if ($isoMetadata.Warning) { Write-WibWarning $isoMetadata.Warning }
        if ($isoMetadata.Mounted -and (-not $isoMetadata.HasBootWim -or (-not $isoMetadata.HasInstallWim -and -not $isoMetadata.HasInstallEsd))) { throw 'ISO создан, но обязательные boot.wim/install.wim/install.esd не найдены.' }
        ...
        Write-Host ''; Write-Host "Готово: $destinationIso" -ForegroundColor Green; Write-Host "SHA-256: $hash"; Write-Host "Лог: $logPath"
        ...
        Write-Host ''; Write-Host ('ОШИБКА СБОРКИ: {0}' -f $_.Exception.Message) -ForegroundColor Red; Write-Host "Рабочий каталог сохранён для продолжения: $workDirectory"; Write-Host "Лог: $logPath"; throw
    }
...
function Invoke-WibBuildPlan {
    ...
    Assert-WibPlan -Plan $Plan
    if ($env:OS -eq 'Windows_NT' -and -not (Test-WibAdministrator)) { Start-WibElevatedPlan -Plan $Plan; return }
