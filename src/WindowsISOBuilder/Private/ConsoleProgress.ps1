...
function Get-WibConverterDetailLogPath {
    if ([string]::IsNullOrWhiteSpace($script:WibConverterBuildLogPath)) { return '' }
    ...
    if ($name.StartsWith('build-', [StringComparison]::OrdinalIgnoreCase)) { $name = 'converter-' + $name.Substring(6) }
    else { $name = 'converter-' + $name }
...
function Write-WibConverterDetailLine {
    ...
    try {
        [IO.File]::AppendAllText($script:WibConverterDetailLogPath, $Line + [Environment]::NewLine, (New-Object Text.UTF8Encoding($false)))
    }
    catch { }
}

function Set-WibConverterProgress {
    param([Parameter(Mandatory = $true)][string]$Status, [int]$Percent = -1)
    if ($Percent -ge 0) {
        $script:WibConverterProgressPercent = [Math]::Max($script:WibConverterProgressPercent, [Math]::Min(99, $Percent))
    }
    Write-Progress -Id $script:WibConverterProgressId -Activity 'Загрузка Windows и создание ISO' -Status $Status -PercentComplete $script:WibConverterProgressPercent
}

function ConvertFrom-WibSpeedText {
    param([AllowNull()][string]$SpeedText)
    if ([string]::IsNullOrWhiteSpace($SpeedText)) { return $null }
    $match = [regex]::Match($SpeedText.Trim(), '(?i)^(?<value>\d+(?:\.\d+)?)(?<unit>B|KiB|MiB|GiB|KB|MB|GB)(?:/s)?$')
    if (-not $match.Success) { return $null }
    $number = 0.0
    if (-not [double]::TryParse($match.Groups['value'].Value, [Globalization.NumberStyles]::Float, [Globalization.CultureInfo]::InvariantCulture, [ref]$number)) { return $null }
    ...
    try { return [int64][Math]::Round($number * $factor) } catch { return $null }
}

# Parse converter output once. Console rendering and structured telemetry consume
# the same normalized progress state; raw converter lines never enter events.
function ConvertFrom-WibConverterProgressLine {
    ...
    if ($percentageMatch.Success) {
        ...
        if ($speedMatch.Success -and $speedMatch.Groups['speed'].Value -ne '0B') {
            $speedText = $speedMatch.Groups['speed'].Value
            $status += ' — ' + $speedText
        }
        return [pscustomobject]@{ Status=$status; Percent=$overallPercent; DetailPercent=$downloadPercent; SpeedText=$speedText; SpeedBytesPerSecond=(ConvertFrom-WibSpeedText $speedText); Stage='download' }
    }
...
function Start-WibConverterProgress {
    ...
    if (-not [string]::IsNullOrWhiteSpace($script:WibConverterDetailLogPath)) {
        try { Remove-Item -LiteralPath $script:WibConverterDetailLogPath -Force -ErrorAction SilentlyContinue } catch { }
        Invoke-WibNativeWriteHost -Object @('[INFO] Подробный вывод конвертера: ' + $script:WibConverterDetailLogPath) -ForegroundColor ([ConsoleColor]::DarkGray)
    }
    Set-WibConverterProgress -Status 'Подготовка загрузки...' -Percent 0
}

function Stop-WibConverterProgress {
    if ($script:WibConverterProgressActive) { Write-Progress -Id $script:WibConverterProgressId -Activity 'Загрузка Windows и создание ISO' -Completed }
    $script:WibConverterProgressActive = $false
    $script:WibConverterProgressPercent = 0
    $script:WibConverterEventStage = ''
}
...
function Update-WibConverterProgressFromLine {
    ...
    $progress = ConvertFrom-WibConverterProgressLine -Line $Line
    if ($null -eq $progress) { return }
    Set-WibConverterProgress -Status $progress.Status -Percent $progress.Percent

    if ($script:WibConverterEventStage -ne $progress.Stage) {
        $script:WibConverterEventStage = $progress.Stage
        Publish-WibEvent -Type 'stage' -Stage $progress.Stage -Message $progress.Status | Out-Null
    }
    $detailPercent = if ($null -eq $progress.DetailPercent) { -1 } else { [int]$progress.DetailPercent }
    $speedBytes = if ($null -eq $progress.SpeedBytesPerSecond) { -1 } else { [int64]$progress.SpeedBytesPerSecond }
    Publish-WibEvent -Type 'progress' -Stage $progress.Stage -Message $progress.Status -Percent ([int]$progress.Percent) -DetailPercent $detailPercent -SpeedText $progress.SpeedText -SpeedBytesPerSecond $speedBytes | Out-Null
}

function Write-Host {
    [CmdletBinding()]
    param(
        [Parameter(Position = 0, ValueFromPipeline = $true)][AllowNull()][object[]]$Object,
        [object]$Separator = ' ',
    ...
    process {
        ...
        $trimmed = $text.Trim()
        if ($trimmed -match '^Лог:\s*(?<path>.+\.log)\s*$') { $script:WibConverterBuildLogPath = $Matches['path'].Trim() }
