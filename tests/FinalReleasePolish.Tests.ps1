Describe 'v1.0.0 final release polish' {
    ...
    It 'keeps catalog rows on theme-owned backgrounds' {
        $xaml = Get-Content (Join-Path $script:repoRoot 'src\WindowsISOBuilder.Gui\Views\CatalogView.xaml') -Raw
    ...
    It 'publishes the GUI as one self-contained file and rejects root DLL clutter' {
        $build = Get-Content (Join-Path $script:repoRoot 'tools\Build-Gui.ps1') -Raw
        $package = Get-Content (Join-Path $script:repoRoot 'tools\New-ReleasePackage.ps1') -Raw
