...
public sealed class EventReaderTests
{
    ...
    [TestMethod] public async Task ReadsUtf8() { var path = Path.GetTempFileName(); try { await File.WriteAllTextAsync(path, Event1 + "\n", new System.Text.UTF8Encoding(false)); var items = await new NdjsonEventReader().ReadNewAsync(path); Assert.AreEqual("Привет", items[0].Message); } finally { File.Delete(path); } }
