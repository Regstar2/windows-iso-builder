...
public sealed class EventReaderTests
{
    ...
    [TestMethod] public async Task UnsupportedSchemaEventIsIgnored() { var path = Path.GetTempFileName(); try { await File.WriteAllTextAsync(path, Event1.Replace("\"schemaVersion\":1", "\"schemaVersion\":2") + "\n"); var items = await new NdjsonEventReader().ReadNewAsync(path); Assert.AreEqual(0, items.Count); } finally { File.Delete(path); } }
    [TestMethod] public async Task EmptyRequestIdEventIsIgnored() { var path = Path.GetTempFileName(); try { await File.WriteAllTextAsync(path, Event1.Replace("\"requestId\":\"r\"", "\"requestId\":\"\"") + "\n"); var items = await new NdjsonEventReader().ReadNewAsync(path); Assert.AreEqual(0, items.Count); } finally { File.Delete(path); } }
    [TestMethod] public async Task EmptyPathReturnsNoEvents() { Assert.AreEqual(0, (await new NdjsonEventReader().ReadNewAsync(string.Empty)).Count); }
}
