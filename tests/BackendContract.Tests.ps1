    ...
    InModuleScope WindowsISOBuilder {
        BeforeEach {
            $script:testBuild = [pscustomobject]@{
                ...
                VersionLabel='25H2'; Build='26200.1'; BuildVersion=[version]'26200.1'; Architecture='amd64';
        ...
        It 'returns GetVersion with distinct application and schema versions' {
            $request = [pscustomobject]@{ schemaVersion=1; requestId='get-version'; command='GetVersion'; arguments=[pscustomobject]@{} }
            $response = Invoke-WibBackendRequestObject $request
            $response.success | Should -BeTrue
            $response.requestId | Should -Be 'get-version'
            $response.applicationVersion | Should -Be '0.2.3-alpha.1'
            $response.data.applicationVersion | Should -Be '0.2.3-alpha.1'
        ...
        It 'accepts schemaVersion 1 and preserves requestId exactly' {
            ...
            $response = Invoke-WibBackendRequestObject ([pscustomobject]@{ schemaVersion=1; requestId=$requestId; command='GetVersion'; arguments=[pscustomobject]@{} })
            $response.requestId | Should -BeExactly $requestId
        }

        It 'rejects unsupported schema with UNSUPPORTED_SCHEMA' {
            try { Invoke-WibBackendRequestObject ([pscustomobject]@{ schemaVersion=2; requestId='x'; command='GetVersion'; arguments=[pscustomobject]@{} }); throw 'expected failure' }
            catch { $_.Exception.Data['WibErrorCode'] | Should -Be 'UNSUPPORTED_SCHEMA' }
        }

        It 'rejects missing command with INVALID_REQUEST' {
            try { Invoke-WibBackendRequestObject ([pscustomobject]@{ schemaVersion=1; requestId='x'; arguments=[pscustomobject]@{} }); throw 'expected failure' }
            catch { $_.Exception.Data['WibErrorCode'] | Should -Be 'INVALID_REQUEST' }
        }

        It 'rejects unknown command with INVALID_COMMAND' {
            try { Invoke-WibBackendRequestObject ([pscustomobject]@{ schemaVersion=1; requestId='x'; command='Invoke-Something'; arguments=[pscustomobject]@{} }); throw 'expected failure' }
            catch { $_.Exception.Data['WibErrorCode'] | Should -Be 'INVALID_COMMAND' }
        }

        It 'rejects invalid command arguments with INVALID_ARGUMENT' {
            try { Invoke-WibBackendRequestObject ([pscustomobject]@{ schemaVersion=1; requestId='x'; command='SearchBuilds'; arguments=[pscustomobject]@{ search=7 } }); throw 'expected failure' }
            catch { $_.Exception.Data['WibErrorCode'] | Should -Be 'INVALID_ARGUMENT' }
        }
        ...
        It 'GetEditions returns edition DTOs' {
            ...
            $response = Invoke-WibBackendRequestObject ([pscustomobject]@{ schemaVersion=1; requestId='editions'; command='GetEditions'; arguments=[pscustomobject]@{ updateId='update-1'; language='ru-ru'; cacheDirectory=$TestDrive } })
            $response.data.editions[0].code | Should -Be 'Professional'
        }
        ...
        It 'ValidateBuildPlan delegates to Assert-WibPlan' {
            ...
            $response = Invoke-WibBackendRequestObject ([pscustomobject]@{ schemaVersion=1; requestId='validate'; command='ValidateBuildPlan'; arguments=[pscustomobject]@{ plan=$planDto } })
        ...
        It 'invalid build plan maps to INVALID_BUILD_PLAN' {
            try { Invoke-WibBackendRequestObject ([pscustomobject]@{ schemaVersion=1; requestId='bad-plan'; command='ValidateBuildPlan'; arguments=[pscustomobject]@{ plan=[pscustomobject]@{} } }); throw 'expected failure' }
        ...
        It 'ExecuteBuildPlan delegates to existing Invoke-WibBuildPlan' {
            ...
            $planDto = ConvertTo-WibBuildPlanDto $script:testPlan
            $response = Invoke-WibBackendRequestObject ([pscustomobject]@{ schemaVersion=1; requestId='execute'; command='ExecuteBuildPlan'; arguments=[pscustomobject]@{ plan=$planDto } })
        ...
        It 'build exception is converted to structured BUILD_FAILED response' {
            ...
            $requestPath = Join-Path $TestDrive 'execute-request.json'
            $responsePath = Join-Path $TestDrive 'execute-response.json'
            $request = [ordered]@{ schemaVersion=1; requestId='execute-error'; command='ExecuteBuildPlan'; arguments=[ordered]@{ plan=ConvertTo-WibBuildPlanDto $script:testPlan } }
            Write-WibJsonFile $request $requestPath 30
        ...
        It 'writes a valid response JSON envelope with expected property names' {
            $requestPath = Join-Path $TestDrive 'version-request.json'
            $responsePath = Join-Path $TestDrive 'version-response.json'
        ...
        It 'does not serialize internal PowerShell metadata into build JSON' {
            Mock Search-WibBuilds { @($script:testBuild) }
            $response = Invoke-WibBackendRequestObject ([pscustomobject]@{ schemaVersion=1; requestId='dto'; command='SearchBuilds'; arguments=[pscustomobject]@{ search='Windows 11'; cacheDirectory=$TestDrive } })
            $json = ConvertTo-WibJsonText $response 20
...
Describe 'Backend Contract static security regressions' {
    It 'uses an explicit command allowlist and no Invoke-Expression' {
        $contractPath = Join-Path $PSScriptRoot '..\src\WindowsISOBuilder\Private\BackendContract.ps1'
        $commandsPath = Join-Path $PSScriptRoot '..\src\WindowsISOBuilder\Private\BackendCommands.ps1'
    ...
    It 'keeps the standalone machine entry point ASCII-only and PS5.1-targeted' {
        $entryPath = Join-Path $PSScriptRoot '..\Invoke-WibBackend.ps1'
        $bytes = [IO.File]::ReadAllBytes($entryPath)
