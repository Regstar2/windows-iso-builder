...
- быстрый режим Windows 11 теперь предпочитает последний стабильный H2-релиз как массовую ветку ежегодного feature-update цикла и не выбирает специализированный H1-релиз только из-за большего номера версии; если H2-релиз в каталоге отсутствует, используется последняя стабильная сборка с предупреждением;
